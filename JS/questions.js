    ...
    constructor(index){
     ...
     display(){
      ...
      >
        ...
        ${this.allAns.map((choise)=> `<li>${choise}</li>`).join('')}
        </ul>
      ...
      let allChoices=Array.from(document.querySelectorAll('.choices li'))
    ...
    nextQuestion(){
       if(this.index<this.questionLength){
        ...
        <div class='text-center text-white animate__animated animate__bounceIn' id='tryAgainContainer'>
                <h1> <span>${myQuiz.score == this.questionLength ? 'Congratulations 🎉 ': `your score is ${myQuiz.score} of ${this.questionLength}`}</span></h1>
